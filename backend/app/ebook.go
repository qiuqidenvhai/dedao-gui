...
// buildEbookInfoFromDetail 使用CatalogList构建完整的书籍信息
func buildEbookInfoFromDetail(detail *services.EbookDetail) *services.EbookInfo {
	info := &services.EbookInfo{}
	
	// 构建TOC目录
	var tocs []utils.EbookToc
	var orders []services.EbookOrders
	
	for i, cat := range detail.CatalogList {
		chapterID := fmt.Sprintf("chapter_%d", i)
		if cat.PlayOrder > 0 {
			chapterID = fmt.Sprintf("chapter_%d", cat.PlayOrder)
		}
		
		toc := utils.EbookToc{
			Href:      chapterID,
			Level:     cat.Level,
			PlayOrder: cat.PlayOrder,
			Text:      cat.Text,
			Offset:    0,
		}
		tocs = append(tocs, toc)
		
		order := services.EbookOrders{
			ChapterID:  chapterID,
			PathInEpub: fmt.Sprintf("OEBPS/%s.xhtml", chapterID),
		}
		orders = append(orders, order)
	}
	
	info.BookInfo.Toc = tocs
	info.BookInfo.Orders = orders
	
	var pages []services.EbookInfoPage
	offset := 0
	for i := range orders {
		page := services.EbookInfoPage{
			Cid:         orders[i].ChapterID,
			PageNum:     i + 1,
			StartOffset: offset,
			EndOffset:   offset + 1000,
		}
		pages = append(pages, page)
		offset += 1000
	}
	info.BookInfo.Pages = pages
	info.BookInfo.EbookBlock = [][]services.EbookBlock{}
...
func EbookInfo(enID string) (info *services.EbookInfo, err error) {
	...
	if detail == nil {
		err = fmt.Errorf("无法获取书籍详情")
		return
	}
	
	// 使用Detail构建EbookInfo
	info = buildEbookInfoFromDetail(detail)
	fmt.Printf("使用Detail API构建书籍信息成功，共 %d 章\n", len(detail.CatalogList))
	return
}

func EbookPage(ctx context.Context, enID string) (info *services.EbookInfo, svgContent utils.SvgContents, err error) {
	// 直接使用Detail API获取书籍信息
	detail, detailErr := getService().EbookDetail(enID)
	if detailErr != nil {
		err = detailErr
		return
	...
	if detail == nil || len(detail.CatalogList) == 0 {
		err = fmt.Errorf("无法获取书籍目录")
		return
	}
	
	// 构建EbookInfo
	info = buildEbookInfoFromDetail(detail)
	fmt.Printf("使用Detail API构建书籍信息，共 %d 章\n", len(detail.CatalogList))
