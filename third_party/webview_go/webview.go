...
// 原生读取 WebView2 Cookie（含 HttpOnly），见 cookies.cc
extern void cookie_request(webview_t w);
extern char *cookie_take();
...
type WebView interface {
	...
	// PollCookies 异步触发一次原生 Cookie 读取（若没有在途请求），并返回
...
// PollCookies 异步发起一次原生 Cookie 读取，并返回上一次读取的结果。
// 因为 GetCookies 是异步的，调用方应在两次 PollCookies 之间留出间隔
// （例如 1.5s 的定时器），第二次调用时通常能拿到第一次发起的结果。
func (w *webview) PollCookies() string {
	// 先取上一次已就绪的结果（C 侧直接返回全局缓冲区指针，避免 Go 主线程栈溢出），
	// 再发起新的异步读取。顺序很关键：若先 request 会把 ready 标志清零，take 拿不到。
	if p := C.cookie_take(); p != nil {
		return C.GoString(p)
	}
	C.cookie_request(w.w)
	return ""
}

// InstallCrashHandler 安装 Windows 顶层异常过滤器：若 C 层段错误导致进程崩溃，
// 把异常码+地址写进 %LOCALAPPDATA%/dedao_login_helper.crash.log，便于无控制台环境定位。
