  ...
  <div class="ebook-container">
    ...
    <div class="ebook-grid-container" v-loading="initLoading" v-infinite-scroll="loadMore" :infinite-scroll-disabled="disabled" :infinite-scroll-immediate="false">
      <div class="ebook-grid">
        <div v-for="item in tableData.list" :key="item.enid" class="ebook-card-wrapper">
          <!-- 多选复选框 -->
          <div v-if="!item.is_group" class="card-checkbox" @click.stop>
            <el-checkbox 
              :model-value="isSelected(item.enid)" 
              @change="toggleSelection(item.enid)"
            />
          </div>
          <div class="ebook-card" :class="{ 'is-selected': isSelected(item.enid) }" @click="handleCardClick(item)">
...
// 多选相关功能
const toggleSelection = (enid: string) => {
    const index = selectedIds.value.indexOf(enid)
    if (index === -1) {
        selectedIds.value.push(enid)
    } else {
...
const isSelected = (enid: string) => {
    return selectedIds.value.includes(enid)
}
...
const downloadSelected = () => {
    ...
    SetDir([setStore.getDownloadDir,
        ...
        const selectedItems = tableData.list.filter((item: any) => 
            !item.is_group && selectedIds.value.includes(item.enid)
        )
