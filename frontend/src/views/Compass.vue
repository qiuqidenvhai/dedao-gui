<template>
  <div class="compass-container">
    ...
    <!-- 批量下载 Dialog -->
    <el-dialog v-model="dialogBatchDownloadVisible" title="批量下载" width="30%" align-center class="custom-dialog">
      <div class="dialog-content">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="dialogBatchDownloadVisible = false">取消</el-button>
...
const dialogBatchDownloadVisible = ref(false)
...
const updateSelectAllState = () => {
    ...
    const totalCount = tableData.list.length
...
const downloadSelected = () => {
    ...
    dialogBatchDownloadVisible.value = true
}

const batchDownload = async () => {
    dialogBatchDownloadVisible.value = false
    
    for (const id of selectedIds.value) {
        try {
            await CourseDownload(id, 0, downloadType.value, '')
        } catch (error) {
            ElMessage({
                message: `下载失败: ${error}`,
                type: 'warning'
    ...
    ElMessage({
        message: `已添加 ${selectedIds.value.length} 个任务到下载队列`,
        type: 'success'
    })
