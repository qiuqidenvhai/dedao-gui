<template>
    <div class="course-container">
        <!-- 多选工具栏 -->
        <div class="selection-toolbar" v-if="!groupMode.active && !searchKeyword">
            <el-checkbox 
                v-model="selectAll" 
        ...
        <div v-if="filterOptions.length > 0 && !groupMode.active && !searchKeyword" class="filter-container">
        ...
        <div v-if="searchKeyword" class="search-header">
            ...
            <span class="search-title">搜索: "{{ searchKeyword }}"</span>
            <span class="search-count">共 {{ total }} 个结果</span>
        </div>
        ...
        <div v-loading="initLoading" class="course-grid-container" v-infinite-scroll="loadMore" :infinite-scroll-disabled="disabled" :infinite-scroll-immediate="false">
            <div v-if="tableData.list && tableData.list.length > 0" class="course-grid">
                <div v-for="item in tableData.list" :key="item.id" class="course-card-wrapper">
                    <!-- 多选复选框 -->
                    <div v-if="!item.is_group" class="card-checkbox" @click.stop>
                        <el-checkbox 
                            :model-value="isCourseSelected(item.id)" 
                            @change="toggleCourseSelection(item.id)"
                        />
                    </div>
                    <div class="course-card" :class="{ 'is-selected': isCourseSelected(item.id) }" @click="item.is_group ? enterGroup(item) : gotoArticleList(item)">
...
const noMore = computed(() => {
    ...
    // 搜索模式或分组模式或筛选模式
    if (searchKeyword.value || groupMode.active || currentFilter.value !== 'all') {
...
const getTableData = async (append = false) => {
    console.log('Course getTableData:', { append, currentFilter: currentFilter.value, groupMode: groupMode.active, searchKeyword: searchKeyword.value })
    loading.value = true
    ...
    await fetcher.then((table) => {
        ...
        console.log('Search/Course results:', table)
...
const downloadSelected = () => {
    ...
    SetDir([setStore.getDownloadDir,
        ...
        const selectedItems = tableData.list.filter((item: any) => 
            !item.is_group && selectedIds.value.includes(String(item.id))
        )
        downloadData.value = selectedItems.map((item: any) => ({
            id: item.class_id,
            aid: 0,
            enid: item.enid,
            title: item.title
