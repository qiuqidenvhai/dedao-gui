<template>
    <div class="breadcrumb-container">
        ...
        <div class="breadcrumb-actions">
            ...
            <el-button 
                ...
                @click="batchDownload"
                class="action-btn batch-download-btn"
            >
                <el-icon><Download /></el-icon>
                批量下载({{ selectedArticles.length }})
            </el-button>
        </div>
    </div>
    ...
    >
        <ul class="article-list">
            <li v-for="item in tableData.article_list" :key="item.id" class="article-card-wrapper">
                <div class="article-card" :class="{ 'is-selected': isSelected(item.id) }">
                    ...
                    <div class="card-main" @click="handlePlay(item)">
    ...
    <download-dialog
            ...
            :en-id="downloadEnId"
            :download-data="batchDownloadData"
...
const batchDownloadData = ref<any[]>([])

const isAllSelected = computed(() => {
    return tableData.article_list.length > 0 && 
           selectedArticles.value.length === tableData.article_list.length
})

const isSelected = (id: number) => {
    return selectedArticles.value.some(item => item.id === id)
}

const toggleSelection = (item: any) => {
    if (isSelected(item.id)) {
        selectedArticles.value = selectedArticles.value.filter(i => i.id !== item.id)
    } else {
...
const batchDownload = async () => {
    ...
    // 准备批量下载数据
    batchDownloadData.value = selectedArticles.value.map(item => ({
        id: item.class_id,
        aid: item.id,
        enid: item.class_enid,
        title: item.title
    }))

    downloadId.value = selectedArticles.value[0]?.class_id || 0
    downloadEnId.value = selectedArticles.value[0]?.class_enid || ''
